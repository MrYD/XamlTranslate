...
namespace XamlTraslate
{
    class Japanese
    {
        public static bool IsConteinsJamanese(string str)
        {
            foreach (var ch in str)
            {
                if (IsHiragana(ch)) return true;
                if (IsKanji(ch.ToString())) return true;
                if (IsKatakana(ch)) return true;
                if (IsKatakanaHalf(ch)) return true;
            }
            return false;
        }
